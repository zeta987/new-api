...
Moonshot model discovery 現在列出官方 `kimi-k3`、`kimi-k2.6`，以及 new-api 本地便利名稱 `kimi-k3-max`、`kimi-k3-none`、`kimi-k2.6-thinking`。官方 model list 沒有定義這些帶 suffix 名稱；它們只屬於 new-api request convenience。來源：[`relay/channel/moonshot/constants.go`](../../relay/channel/moonshot/constants.go)、[Kimi 模型列表](https://platform.kimi.com/docs/models)。
