...
**Versioned self-use release workflow:** This is the highest-priority release maintenance rule. The decisions below are the authoritative development, carry-forward, deployment, and rollback policy as of 2026-07-20.

1. **Use one development branch per upstream version.** For upstream tag `v<version>`, use `dev/v<version>`. Create the development branch from the completed matching `release/v<version>` branch. When a new upstream version is integrated, create a new versioned development branch; do not reuse the old development branch across versions.

2. **Separate working branches from reusable backups.** Create active work from the matching development branch as `feat/<rc-name>/<feature-name>` or `fix/<rc-name>/<fix-name>` (for example, `feat/rc21/...` and `fix/rc21/...`). Reserve `feat/v<version>/...` and `fix/v<version>/...` for tag-based, reusable carry-forward backups.

3. **Do not make ordinary feature or fix commits directly on development or release branches.** Features and fixes must enter through their working branches. Direct commits on development or release are limited to merge-conflict resolution, release/governance documentation, and mechanical release adjustments that cannot reasonably live in a topic branch.

4. **Use different merge policies for integration and release.** Merge `feat/...` and `fix/...` into the matching development branch with an ordinary merge; `--no-ff` is not required. Keep the topic commits logically organized and signed, and do not squash at merge time. The repository uses `commit.gpgsign=true` with SSH signing, so a merge commit created naturally when fast-forward is unavailable is signed automatically. Promote development to release with a signed `git merge --no-ff -S`, and verify that release merge signature.

5. **Apply a scope-aware release test gate.** Every promotion must pass `git diff --check`, focused tests for the changed behavior, and a local runtime smoke test. Backend changes additionally require `go test ./...`, `go build ./...`, and `go vet ./...`. Changes under `web/default` require `bun run lint`, `bun run format:check`, and `bun run build:check`; changes to classic or shared web code also require the classic frontend build. A broad-test failure may be treated as pre-existing only when it reproduces on the unmodified release base and is recorded. All content in the development branch is one release candidate: if any included change fails, fix it, wait, or revert it and rerun the gate; never cherry-pick only selected development commits into release.

6. **Deploy from the versioned release branch that matches the upstream tag.** Zeabur monitors the active `release/v<version>` branch directly. While upstream remains on `v1.0.0-rc.21`, use `dev/v1.0.0-rc.21` for integration and `release/v1.0.0-rc.21` for production. Change the monitored branch only after the next version has completed its release procedure. Two Zeabur projects are bound to this repository: the project named `GCP-TW_AI-Rel` is the Release deployment and auto-redeploys on pushes to the active `release/v<version>` branch; the project named `GCP-TW_AI-Dev` is the Dev deployment and auto-redeploys on pushes to the active `dev/v<version>` branch. Do not record raw Zeabur project IDs in this repository; resolve them at runtime by searching these project names with the Zeabur skills/CLI (`npx zeabur@latest project list`). The Dev Zeabur project is manually suspended after each testing round; when a deployment-level test is needed, resume that project first via the Zeabur skills/CLI, run the test, and expect the owner to suspend it again afterward. Both projects front the `New-API` service with an Anubis proof-of-work reverse proxy (production entry `new-api.zeta987.com` is bound to the `anubis` service, which forwards to the New-API service over private networking; API relay paths `/v1`, `/v1beta`, `/mj`, `/suno`, `/kling` are allowlisted so AI clients are never challenged).

7. **Carry only released behavior to a new upstream version.** Do not move an old development branch wholesale. Work that remains only in `dev/v<old-version>` or its active topic branches is not carried automatically. Evaluate unfinished work individually and create a corresponding topic branch for the new development version only when it should continue.

8. **Back up by theme, and inventory by customization.** Backup branches are grouped slices, not one branch per model or per commit. A theme collects customizations that solve the same problem and edit the same files, so the shared plumbing lives in exactly one branch instead of being split hunk by hunk across several. The active themes are `feat/v<version>/reasoning-model-support` (model-name reasoning effort and thinking suffixes across every provider, plus the pricing, model-matching, and channel-selection plumbing they share), `feat/v<version>/chatcompletions-responses-compat`, `fix/v<version>/usage-logs-realtime-refresh`, `fix/v<version>/channel-affinity-test-isolation`, and `fix/v<version>/postgres-automigrate-compat` (PostgreSQL GORM named-unique migration compatibility). The first four themes were selected by the user on 2026-08-19; the PostgreSQL migration theme was added on 2026-08-31 after the verified rc29 GORM v1.25.12 deployment failure. The released model-pricing deletion customization also uses `fix/v<version>/model-pricing-deletion`. From rc.34, `fix/v<version>/frontend-upgrade-compat` groups upstream frontend compatibility corrections and their scoped validation/formatting repairs, including Combobox interaction inside modal drawers. These two themes address UI editing and upstream integration behavior outside the original five themes. Add a theme only when a customization fits none of the existing ones. The inspection checklist stays per customization: on an upstream version update, evaluate every released feature and standalone fix individually even when several share one backup branch, and retire them individually inside their theme. A checklist entry is not an unconditional instruction to recreate its branch.

9. **Inspect upstream before carrying a customization.** For every checklist item, compare the new upstream behavior with the released customization. If upstream lacks it, carry the complete customization. If upstream implements only part of it, carry only the remaining delta. If upstream implements the full required behavior, retire the customization for the new version and do not create a corresponding new backup branch. The mandatory minimum is to inspect and guarantee the behavior of OpenAI Chat Completions / Responses compatibility (including OpenAI and x.ai tool compatibility) and Claude adaptive thinking / newer Claude model support; it is not an unconditional requirement to recreate same-named branches.

10. **Require evidence for automatic retirement.** Normal retirement requires both code-path comparison and the existing regression tests to prove that the clean new upstream base provides equivalent behavior. Partial equivalence produces a delta-only backup. A user may also explicitly declare a customization retired; record the decision and date without requiring the normal dual evidence. A retired customization does not receive a backup branch for the new active version, and superseded-version refs follow the pruning policy in rule 17 instead of serving as permanent archives. The classic `zh-TW` locale customization was explicitly retired by the user on 2026-07-18: do not inspect it during future upgrades, do not create rc22-or-later locale backup branches, and leave all upstream-provided `zh-TW` files under `web/classic`, `web/default`, `i18n`, and `README.zh_TW.md` unchanged.

11. **Complete backups before triggering production.** After the signed development-to-release merge, run the release gate, create or update every required versioned themed backup, verify their diffs and signatures, and push the backup branches first. Push the active release branch last so that production deployment cannot begin before its reusable slices exist remotely.

12. **Append within a version; rebuild across versions.** When a released feature or fix changes again within the same upstream version, append signed delta commits to its theme's existing versioned backup and verify that the branch still contains only that theme's customizations. A new customization that belongs to an existing theme is appended to that theme rather than given its own branch. Do not force-push published backup history. For a new upstream version, create a new clean backup from the new upstream tag after the fully integrated release is complete.

13. **Delete working branches only after local and remote verification.** Treat `feat/<rc-name>/...` and `fix/<rc-name>/...` as construction branches. Delete one locally and on origin only after development tests pass, the behavior is in release and verified there, the corresponding backup is created/updated and pushed (or the behavior is validly retired), and the remote release and backup refs point to the expected commits. Preserve branches that are unmerged, failing, paused, awaiting a decision, or still subject to change. Deleting a working ref must not make its commits unreachable from development, release, or a versioned backup.

14. **Keep `main` as a pure upstream mirror.** `main` receives only fork synchronization from upstream and never accepts merges from development, release, feature, fix, or any other customization branch. It is not a release, test, or deployment branch, and a push to `main` must not trigger Zeabur.

15. **Create a new release from the prior integrated release.** For upstream `v<new-version>`, create `release/v<new-version>` from the previous `release/v<old-version>`, then merge the new upstream tag. Resolve conflicts, inspect carry-forward items, run tests, rebuild the required backups from the new tag, and verify signatures. Create `dev/v<new-version>` only from the completed new release.

16. **Switch Zeabur only after remote verification.** A new versioned release may be pushed before Zeabur monitors it. Push and verify all new backup refs and the new release ref first; then change Zeabur from the old versioned release branch to the new one. After switching, inspect build logs, runtime logs, service metrics, and health. Apply the active-version branch retention policy in rule 17 only after the new deployment is healthy.

17. **Keep only the active version's branch set.** After the new deployment is healthy and its release and backup refs are verified locally and remotely, retain only `main`, `dev/v<active-version>`, `release/v<active-version>`, and the required themed `feat/v<active-version>/...` and `fix/v<active-version>/...` backups named in rule 8. Delete superseded versioned development, release, and backup branches from both local and origin, together with obsolete unversioned source branches. Local `pr/...` inspection refs are outside this cleanup unless the user explicitly includes them. Do not delete upstream tags. Branch pruning requires an explicit user decision and an exact target inventory immediately before deletion. The user selected this active-version-only policy on 2026-07-20; the active version at that time was `v1.0.0-rc.21`.

18. **Use a two-stage production rollback.** First use `zeabur-deployment-logs` and `zeabur-service-metric` to confirm the failed deployment. When immediate recovery is required, use Zeabur Dashboard rollback to the previous successful snapshot while keeping Zeabur pointed at the current release branch. Recheck logs, metrics, and health, then repair through `fix/<rc-name>/... -> dev/v<version> -> release/v<version>`. A prior release branch can be selected only before rule 17 pruning; afterward, an explicit user decision is required to recreate a temporary branch from a recorded signed release commit when that object remains available. Application rollback does not restore databases, volumes, or environment variables.

19. **Treat schema changes as a pre-push code-review gate.** This project executes GORM `AutoMigrate` from `model/main.go` when a new deployment starts, so there is no separate migration phase. Validate schema changes on SQLite, MySQL, and PostgreSQL. Custom migrations may only be additive (new tables, columns, or indexes); destructive rename, drop, or type changes requiring raw SQL must not enter release without a separately approved and recorded plan. Before pushing a monitored release with schema changes, back up the production database and record restoration steps. Additive schema must remain readable by the prior application; backfills or semantic changes require explicit compatibility evidence. Apply the same checks to schema changes brought in by an upstream tag before switching Zeabur to that release. If data compatibility cannot be demonstrated, do not push the release.

20. **Require the exact production confirmation phrase.** Before any push to the release branch currently monitored by Zeabur, present the test commands/results, release SHA, backup refs, signature verification, and, when applicable, production database backup status. Do not push until the user enters exactly `確認發布`. This gate also applies to emergency fixes and post-rollback redeployments without exception. A new release branch not yet monitored by Zeabur may be pushed and verified under rule 16 without this production confirmation.

21. **Promote development as one indivisible release candidate.** Everything already merged into `dev/v<version>` is promoted together. A failing or unapproved item must be fixed, delayed with the whole candidate, or reverted from development before retesting. Never select individual development commits for direct release promotion. Merge a topic into development only after its own focused tests pass to reduce candidate-wide blocking.

22. **Realign development after a release.** Immediately after the confirmed release push, run `git merge --ff-only release/v<version>` on the matching development branch; the signed release merge normally has the prior development tip as its second parent, so this fast-forward should succeed. If new work entered development before synchronization and fast-forward is unavailable, merge release back into development normally without rewriting history. An unpublished, local-only topic may rebase onto the updated development branch. A topic already pushed to origin or used by another agent must merge development instead and must never be force-pushed.

23. **Use the local authenticated browser regression procedure.** For regressions that cross the API and default frontend, start the backend from the repository root with `go run main.go` and start the default frontend from `web/default` with `bun run dev -- --host 127.0.0.1 --port 5173`. Observe the affected page with the Codex in-app browser and exercise the real local API path with `curl` or an equivalent command; for usage-log refresh checks, send a real `/v1/chat/completions` request with the requested model and verify both backend log creation and the already-open page's network/UI update. If the browser requires authentication, pause the test and wait for the user to log in manually before continuing; do not inspect browser storage, extract session credentials, or bypass login. Never print local API tokens in command output or documentation. After automated checks finish, keep every backend and frontend development server running so the user can perform manual verification. Stop those servers only after the user explicitly confirms that manual testing is complete or explicitly asks for them to be stopped, then verify that their listening ports have been released. Event-driven behavior must be tested from the triggering request itself rather than replaced with fixed-interval data polling.
