...
**Versioned self-use release workflow:** This is the highest-priority release maintenance rule. The decisions below are the authoritative development, carry-forward, deployment, and rollback policy as of 2026-07-18.
...
10. **Require evidence for automatic retirement, while preserving history.** Normal retirement requires both code-path comparison and the existing regression tests to prove that the clean new upstream base provides equivalent behavior. Partial equivalence produces a delta-only backup. A user may also explicitly declare a customization retired; record the decision and date without requiring the normal dual evidence. Retirement never deletes or rewrites an older version's backup refs. The classic `zh-TW` locale customization was explicitly retired by the user on 2026-07-18: do not inspect it during future upgrades and do not create rc22-or-later locale backup branches. Preserve and freeze `feat/v1.0.0-rc.14/classic-zh-tw-locale-translations` and `feat/v0-13-2-zh-tw-locale-translations`, and leave all upstream-provided `zh-TW` files under `web/classic`, `web/default`, `i18n`, and `README.zh_TW.md` unchanged.
...
16. **Switch Zeabur only after remote verification.** A new versioned release may be pushed before Zeabur monitors it. Push and verify all new backup refs and the new release ref first; then change Zeabur from the old versioned release branch to the new one. After switching, inspect build logs, runtime logs, service metrics, and health. Keep every old release and backup ref.

17. **Freeze superseded versions.** After the new deployment is healthy, stop advancing the old release, development, and versioned backup refs. Freezing is a governance rule; GitHub's lock-branch protection may be enabled when explicitly requested. The only exception is an explicit production rollback to the old version that requires an emergency fix, which must follow the same signed fix, backup, test, and release rules.

18. **Use a two-stage production rollback.** First use `zeabur-deployment-logs` and `zeabur-service-metric` to confirm the failed deployment. When immediate recovery is required, use Zeabur Dashboard rollback to the previous successful snapshot while keeping Zeabur pointed at the current release branch. Recheck logs, metrics, and health, then repair through `fix/<rc-name>/... -> dev/v<version> -> release/v<version>`. Change the monitored branch back to the frozen prior release only when the user explicitly pauses the new version. Application rollback does not restore databases, volumes, or environment variables.
