...
8. **Back up by theme, and inventory by customization.** Backup branches are grouped slices, not one branch per model or per commit. A theme collects customizations that solve the same problem and edit the same files, so the shared plumbing lives in exactly one branch instead of being split hunk by hunk across several. The active themes are `feat/v<version>/reasoning-model-support` (model-name reasoning effort and thinking suffixes across every provider, plus the pricing, model-matching, and channel-selection plumbing they share), `feat/v<version>/chatcompletions-responses-compat`, `fix/v<version>/usage-logs-realtime-refresh`, `fix/v<version>/channel-affinity-test-isolation`, and `fix/v<version>/postgres-automigrate-compat` (PostgreSQL GORM named-unique migration compatibility). The first four themes were selected by the user on 2026-08-19; the PostgreSQL migration theme was added on 2026-08-31 after the verified rc29 GORM v1.25.12 deployment failure. Add a theme only when a customization fits none of the existing ones. The inspection checklist stays per customization: on an upstream version update, evaluate every released feature and standalone fix individually even when several share one backup branch, and retire them individually inside their theme. A checklist entry is not an unconditional instruction to recreate its branch.
