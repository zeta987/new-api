...
function renderProvider(
  ...
  child = <div>usage logs</div>
) {
  return render(
    <QueryClientProvider client={queryClient}>
      <UsageLogsProvider>{child}</UsageLogsProvider>
    </QueryClientProvider>
  )
}
...
describe('usage logs provider stream authentication', () => {
  ...
  test('replaces the stream with current headers after token rotation', () => {
    ...
    unmount()
    expect(streamTestState.events.at(-1)).toBe('close:Bearer token-b')
  })
...
describe('usage logs background refresh', () => {
  let queryClient: QueryClient
  let navigateToErrorPage = vi.fn<() => void>()
  ...
  beforeEach(() => {
    ...
    queryClient = createAppQueryClient(navigateToErrorPage)
      ...
      if (Date.now() < rejectUntil) {
        throw new AxiosError(
          ...
          undefined,
          {
            ...
            statusText: 'Background refresh failed',
          }
        )
      }
      return {
  ...
  function renderLiveLogs() {
    return renderProvider(queryClient, <LiveLogsProbe />)
  }

  test('coalesces successive events into one refresh and does not poll afterward', async () => {
    renderLiveLogs()
  ...
  test('waits through Retry-After, catches up, and keeps throttling silent', async () => {
  ...
  test('keeps hidden tabs idle and refreshes after becoming visible', async () => {
    renderLiveLogs()
