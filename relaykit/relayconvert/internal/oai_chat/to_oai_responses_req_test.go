...
func TestChatCompletionsRequestToResponsesRequestPreservesReasoningExtensions(t *testing.T) {
	var req dto.GeneralOpenAIRequest
	require.NoError(t, kitutil.Unmarshal([]byte(`{
		"model":"gpt-5.6",
		"messages":[{"role":"user","content":"hello"}],
		"reasoning":{"effort":"high","summary":"concise","mode":"pro","context":{"trace_id":"trace-1"}}
	}`), &req))
	...
	assert.JSONEq(t, `"pro"`, string(got.Reasoning.Mode))
	assert.JSONEq(t, `{"trace_id":"trace-1"}`, string(got.Reasoning.Context))
}
