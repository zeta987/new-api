...
func TestGLM52ZhipuV4ForwardsReasoningEffortForNonGLMModels(t *testing.T) {
	for _, model := range []string{"custom-model", "GLM-5.2", "custom-glm-5.2"} {
		t.Run(model, func(t *testing.T) {
			...
			assert.Equal(t, "high", got.ReasoningEffort)
			assert.Empty(t, info.ReasoningEffort)
			assert.JSONEq(t, `{"model":"`+model+`","reasoning_effort":"high","stop":null}`, marshalRequest(t, got))
		})
	}
}
...
func TestGLM52ZhipuV4ClearsReusedReasoningEffortMetadata(t *testing.T) {
	...
	assert.Equal(t, "high", got.ReasoningEffort)
	assert.Empty(t, info.ReasoningEffort)
	assert.JSONEq(t, `{"model":"custom-model","reasoning_effort":"high","stop":null}`, marshalRequest(t, got))
}
