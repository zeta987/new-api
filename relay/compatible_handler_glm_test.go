...
func TestQwenAliasesRelayResponsesEffortAndOverrideFunctionTools(t *testing.T) {
	...
	for _, base := range []string{"qwen3.8-max", "qwen3.8-flash"} {
		for _, effort := range []string{"none", "low", "medium", "xhigh"} {
			t.Run(base+"-"+effort, func(t *testing.T) {
				...
				require.NoError(t, common.UnmarshalJsonStr(`{"operations":[{"mode":"set","path":"tools","value":[{"type":"function","function":{"name":"lookup","description":"Look up information","parameters":{"type":"object","properties":{}}}}],"conditions":[{"path":"messages.-1.content","mode":"contains","value":"搜尋"}],"logic":"OR"}]}`, &info.ParamOverride))
				...
				case upstream := <-captured:
					...
					assert.Equal(t, []any{map[string]any{"type": "function", "name": "lookup", "description": "Look up information", "parameters": map[string]any{"type": "object", "properties": map[string]any{}}}}, body["tools"])
				default:
...
func TestOpenRouterAliasesRelayMappedResponsesFunctionTools(t *testing.T) {
	policy := model_setting.ChatCompletionsToResponsesPolicy{
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.alias, func(t *testing.T) {
					...
					ParamOverride: map[string]interface{}{
						"operations": []interface{}{
							...
							map[string]interface{}{"mode": "set", "path": "tools", "value": []interface{}{
								map[string]interface{}{"type": "function", "function": map[string]interface{}{"name": "calculate", "parameters": map[string]interface{}{"type": "object"}}},
							}},
						},
					},
				},
			...
			case upstream := <-captured:
				...
				assert.Equal(t, []interface{}{map[string]interface{}{"type": "function", "name": "calculate", "parameters": map[string]interface{}{"type": "object"}}}, body["tools"])
			default:
				t.Fatal("upstream did not receive a Responses request")
			}
		})
