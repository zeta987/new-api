...
Local authenticated browser verification covered real gateway Chat and Responses requests against an isolated mock upstream, effort/model normalization, billing log creation, automatic updates on an already-open page, and preservation of the open details dialog. The rollout retains upstream's default Argon2id for newly set passwords; existing bcrypt hashes remain readable. The rollback-reader tests cover the preserved pre-upgrade data. Passwords created or changed after deployment require an Argon2id-capable application and cannot be verified by unmodified rc.33; restoring the database is a separate explicit recovery operation. Deployment receipts and the protected database archive are retained in the ignored local release evidence directory.
